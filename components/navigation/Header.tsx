...
export function Header() {
  ...
  return (
    <header className="h-16 border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-20 px-4 sm:px-8 flex items-center justify-between">
      {/* Mobile Brand / Page Indicator */}
      <div className="flex items-center gap-3">
        <div className="lg:hidden w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white shadow-sm">
          <GraduationCap className="w-5 h-5" />
        </div>
        <div>
          <h2 className="text-sm sm:text-base font-semibold text-slate-800 tracking-tight flex items-center gap-2">
            Tuition Workspace
            <span className="hidden sm:inline-block text-[11px] font-normal text-slate-400 border-l border-slate-200 pl-2">
              {todayStr}
            </span>
          </h2>
        </div>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <Link
          ...
          target="_blank"
          rel="noreferrer"
          className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 transition-colors"
        >
                ...
                <div className="py-2 space-y-2 max-h-80 overflow-y-auto">
                  {insights.alerts.length === 0 ? (
                    ...
                    insights.alerts.map((alert, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100/70 transition-colors text-xs"
                      >
